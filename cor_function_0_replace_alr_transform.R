...
dataV9X <- fread("globaldataset.otus.v20171106.withfunctions.pub.tsv")
names(dataV9X)[-1] = sub('.*\\;', "", names(dataV9X)[-1]) ####This line is actually essential for the official dataset with V9 data
dataV9_meltedX <- dataV9X[,.SD,.SDcols=c("md5sum",grep("TV9|TA9",colnames(dataV9X),value=T))] %>%
  melt(id.vars="md5sum")
#reading and reshaping V9 dataset

#counting relative abundances of V9 metabarcodes

dataV9_meltedX[,value2:=value/sum(value),by=variable]

#counting relative abundances of V9 metabarcodes


#0 replacement by smallest value

zeros <- which(dataV9_meltedX$value2 == 0)
dataV9_meltedX[zeros, 4] <- min(na.omit(data.frame(dataV9_meltedX[!zeros,4])))

#0 replacement by smallest value

#Centred log-ratio transformation of V9 barcode abundances

#dataV9_meltedX[,value3:=value2+1]
#dataV9_meltedX[,value3:=compositions::clr(value3),by=variable]

#Centred log-ratio transformation of V9 barcode abundances

#removing the untransformed value

#Additive-log-ratio transformation of abundances

dataV9_meltedX[,value3:=value2+1]
dataV9_meltedX[,value3:=compositions::alrInv(value3)[1:474303],by=variable]


#Additive-log-ratio transformation of abundances


dataV9_meltedX[,value:=NULL]
dataV9_meltedX[,value2:=NULL]
#removing the untransformed value
...
for(i in list_of_all[1:(length(list_of_all))]) {
  ...
  dataSMAGs <- melt(dataSMAGs,id.vars="SMAG")#first melting the dataframe
  
  #0 replacement by smallest value
  
  zeros <- which(dataSMAGs$value == 0)
  dataSMAGs[zeros, 3] <- min(na.omit(data.frame(dataSMAGs[!zeros,3])))
...
#  dataSMAGs <- melt(dataSMAGs,id.vars="SMAG")#first melting the dataframe
  ...
  dataSMAGs[,value2:=value+1]
  #dataSMAGs[,value2:=compositions::clr(value2),by=variable]
  dataSMAGs[,value2:=compositions::alrInv(value2)[1:713],by=variable]
  ...
  #dataSMAGs[,value:=NULL]
  
  #removing the untransformed value
  
  #reshaping the dataframe and matching station IDs with metagenome IDs
  
  dataSMAGs_melted <- dataSMAGs #melt(dataSMAGs,id.vars="SMAG") #-in this script, we melted it above, to carry out the transformation
  ...
  dataSMAGs_melted$variable <- sub("X", "", dataSMAGs_melted$variable)
  
  dataSMAGs_melted <- merge(dataSMAGs_melted,tmp,by.x="variable",by.y="V1",all.x = T)
  dataSMAGs_df <- dcast(Sample.mat~SMAG,value.var="value2",data=dataSMAGs_melted[!is.na(Sample.mat)],fun.aggregate = mean) %>%
  ...
  #zeros <- which(dataV9_melted$value2 == 0)
  #dataV9_melted[zeros, 4] <- min(na.omit(data.frame(dataV9_melted[!zeros,4])))
  
  #0 replacement by smallest value
  
  #Centred log-ratio transformation of V9 barcode abundances
  
  #dataV9_melted[,value3:=value2+1]
  #dataV9_melted[,value3:=compositions::clr(value3),by=variable]
  
  #Centred log-ratio transformation of V9 barcode abundances
  
  #removing the untransformed value
  
  #dataV9_melted[,value:=NULL]
  #dataV9_melted[,value2:=NULL]
  #removing the untransformed value
  ...
  dataV9 <- dataV9X[taxogroup==c(taxogroup_i)[1]&pid>90]
  
  x <- apply(dataV9[,.SD,.SDcols=grep("TV9|TA9",colnames(dataV9),value=T)],1,function(X) sum(X>0))
  dataV9 <- dataV9[x>100]
  dataV9_melted <- dataV9_meltedX[md5sum%in%dataV9[,md5sum]]
  ...
  dataV9_df_relabund <- dcast(Sample.mat~md5sum,value.var="value3",data=dataV9_melted,fun.aggregate = mean) %>%
  ...
  if (length(levels(as.factor(data_cor$SMAG))) == 1) {
    ...
    for (e in c(1:length(data_cor$md5sum))) {
      ...
      #y <- which(tmp[,1] != 0)
      x <- which(tmp[,data_cor$md5sum[e]] != min(tmp[,data_cor$md5sum]))#altered from 0 to minimum - because of the 0 substitution procedure
      y <- which(tmp[,1] != min(tmp[,1]))
